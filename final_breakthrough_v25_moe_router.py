...
def apply_env_overrides() -> None:
    ...
    if "V25_PSEUDO_CONF" in os.environ:
    ...
    if "V25_PSEUDO_ENT" in os.environ:
    ...
    if "V25_PSEUDO_MAX_RATIO" in os.environ:
    ...
    if "V25_PSEUDO_MIN_COUNT" in os.environ:
    ...
    if "V25_PSEUDO_WEIGHT" in os.environ:
    ...
    if "V25_NOISE_REWEIGHT" in os.environ:
    ...
    if "V25_MOE_ROUTER" in os.environ:
...
def main() -> None:
    ...
    run_id = f"v25-{now_utc.strftime('%Y%m%d-%H%M%S')}"
    ...
    print(f"Running v25 on feature={feature_device}, train={train_device}; run_id={run_id}; cfg={run_hash}")
    ...
    full_mode = os.getenv("V25_FULL", os.getenv("V22_FULL", os.getenv("V20_FULL", os.getenv("V17_FULL", "0")))) == "1"
    if lightweight:
    ...
    if lightweight:
        ...
        print(f"V25 full mode enabled: tta={tta_keys}, seeds={probe_seeds}, full_pseudo={full_pseudo}")
    else:
    ...
    if lightweight:
        ...
        if not CFG.use_species_adapter:
            print("Stage: species adapters disabled (v25 default)", flush=True)
    ...
    submission.to_csv("submission_final_team_v25.csv", index=False)
    ...
    pseudo_df.to_csv("v25_pseudo_labels_consensus.csv", index=False)
    ...
    with open("v25_oof_metrics.json", "w", encoding="utf-8") as f:
    ...
    manifest = {
        ...
        "outputs": {
            "final_submission": "submission_final_team_v25.csv",
            "metrics": "v25_oof_metrics.json",
            "pseudo_labels": "v25_pseudo_labels_consensus.csv",
        },
    }

    with open("v25_run_manifest.json", "w", encoding="utf-8") as f:
    ...
    print("Generated: submission_final_team_v25.csv")
    print("Generated: v25_oof_metrics.json")
    print("Generated: v25_run_manifest.json")
