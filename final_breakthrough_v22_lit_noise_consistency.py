...
def apply_env_overrides() -> None:
    ...
    if "V22_PSEUDO_CONF" in os.environ:
    ...
    if "V22_PSEUDO_ENT" in os.environ:
    ...
    if "V22_PSEUDO_MAX_RATIO" in os.environ:
    ...
    if "V22_PSEUDO_MIN_COUNT" in os.environ:
    ...
    if "V22_PSEUDO_WEIGHT" in os.environ:
    ...
    if "V22_TTA_GATE" in os.environ:
        CFG.tta_gate_enabled = os.environ["V22_TTA_GATE"] == "1"
    if "V22_NOISE_REWEIGHT" in os.environ:
...
def main() -> None:
    ...
    run_id = f"v22-{now_utc.strftime('%Y%m%d-%H%M%S')}"
    ...
    print(f"Running v22 on feature={feature_device}, train={train_device}; run_id={run_id}; cfg={run_hash}")
    ...
    full_mode = os.getenv("V22_FULL", os.getenv("V20_FULL", os.getenv("V17_FULL", "0"))) == "1"
    if lightweight:
    ...
    full_pseudo = os.getenv("V22_FULL_PSEUDO", os.getenv("V20_FULL_PSEUDO", os.getenv("V17_FULL_PSEUDO", "0"))) == "1"
    if lightweight:
        tta_keys = ["orig"]
        probe_seeds = (42,)
    elif full_mode:
        ...
        print(f"V22 full mode enabled: tta={tta_keys}, seeds={probe_seeds}, full_pseudo={full_pseudo}")
    else:
    ...
    if lightweight:
        ...
        if not CFG.use_species_adapter:
            print("Stage: species adapters disabled (v22 default)", flush=True)
    ...
    submission.to_csv("submission_final_team_v22.csv", index=False)
    ...
    pseudo_df.to_csv("v22_pseudo_labels_consensus.csv", index=False)
    ...
    with open("v22_oof_metrics.json", "w", encoding="utf-8") as f:
    ...
    manifest = {
        ...
        "outputs": {
            "final_submission": "submission_final_team_v22.csv",
            "metrics": "v22_oof_metrics.json",
            "pseudo_labels": "v22_pseudo_labels_consensus.csv",
        },
    }

    with open("v22_run_manifest.json", "w", encoding="utf-8") as f:
    ...
    print("Generated: submission_final_team_v22.csv")
    print("Generated: v22_oof_metrics.json")
    print("Generated: v22_run_manifest.json")
