...
def maybe_trackio_init(run_name: str):
    if trackio is None:
        return False
    try:
        trackio.init(project="pet-expression-v12", run_name=run_name, config=asdict(CFG))
        return True
    except Exception:
        return False
...
def main() -> None:
    ...
    run_id = f"v12-{now_utc.strftime('%Y%m%d-%H%M%S')}"
    run_hash = config_hash()

    device = torch.device("cuda" if torch.cuda.is_available() else ("mps" if torch.backends.mps.is_available() else "cpu"))
    print(f"Running v12 on {device}; run_id={run_id}; cfg={run_hash}")
    ...
    submission.to_csv("submission_final_team_v12.csv", index=False)
    ...
    pseudo_df.to_csv("v12_pseudo_labels_consensus.csv", index=False)
    ...
    with open("v12_oof_metrics.json", "w", encoding="utf-8") as f:
    ...
    manifest = {
        ...
        "outputs": {
            "final_submission": "submission_final_team_v12.csv",
            "metrics": "v12_oof_metrics.json",
            "pseudo_labels": "v12_pseudo_labels_consensus.csv",
        },
    }

    with open("v12_run_manifest.json", "w", encoding="utf-8") as f:
    ...
    print("Generated: submission_final_team_v12.csv")
    print("Generated: v12_oof_metrics.json")
    print("Generated: v12_run_manifest.json")
