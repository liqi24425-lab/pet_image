...
class V10Config:
    ...
    probe_seeds: Tuple[int, ...] = (42, 52, 62)
...
def maybe_trackio_init(run_name: str):
    if trackio is None:
        return False
    try:
        trackio.init(project="pet-expression-v16", run_name=run_name, config=asdict(CFG))
        return True
    except Exception:
        return False
...
def main() -> None:
    ...
    run_id = f"v16-{now_utc.strftime('%Y%m%d-%H%M%S')}"
    run_hash = config_hash()

    device = torch.device("cuda" if torch.cuda.is_available() else ("mps" if torch.backends.mps.is_available() else "cpu"))
    print(f"Running v16 on {device}; run_id={run_id}; cfg={run_hash}")
    ...
    tta_keys = ["orig", "flip", "roi", "roi_flip"]
    ...
    for name in ["dino", "effnet", "clip"]:
        for tta in tta_keys:
            member_probs = [
                predict_proba_linear_probe(model, test_features[name][tta], device) for model in full_models[name]
            ]
    ...
    submission.to_csv("submission_final_team_v16.csv", index=False)
    ...
    pseudo_df.to_csv("v16_pseudo_labels_consensus.csv", index=False)
    ...
    with open("v16_oof_metrics.json", "w", encoding="utf-8") as f:
    ...
    manifest = {
        ...
        "outputs": {
            "final_submission": "submission_final_team_v16.csv",
            "metrics": "v16_oof_metrics.json",
            "pseudo_labels": "v16_pseudo_labels_consensus.csv",
        },
    }

    with open("v16_run_manifest.json", "w", encoding="utf-8") as f:
    ...
    print("Generated: submission_final_team_v16.csv")
    print("Generated: v16_oof_metrics.json")
    print("Generated: v16_run_manifest.json")
