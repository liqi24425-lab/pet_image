...
class AdaptivePetCrop:
    ...
    def __call__(self, image: Image.Image) -> Image.Image:
        ...
        # Gradient-based saliency map (simple and fast).
        ...
        ys = range(0, h - win + 1, self.stride)
        xs = range(0, w - win + 1, self.stride)
        for y in ys:
            for x in xs:
        ...
        # Fallback to center if saliency is too weak/noisy.
        if best_score < 0.05:
            x = (w - win) // 2
            y = (h - win) // 2
        else:
...
def build_transforms() -> Dict[str, Dict[str, transforms.Compose]]:
    ...
    def make_ttas(norm):
        return {
            ...
            "roi": transforms.Compose(
                [adaptive_crop, transforms.ToTensor(), norm]
            ),
...
def gate_tta_probs(per_tta_probs: List[np.ndarray]) -> np.ndarray:
    """Select per-sample most-confident TTA prediction instead of plain averaging."""
...
def main() -> None:
    ...
    run_id = f"v19-{now_utc.strftime('%Y%m%d-%H%M%S')}"
    ...
    print(f"Running v19 on feature={feature_device}, train={train_device}; run_id={run_id}; cfg={run_hash}")
    ...
    tta_keys = ["orig"] if lightweight else ["orig", "roi"]
    ...
    if CFG.tta_gate_enabled:
        test_probs_by_model = {name: gate_tta_probs(probs) for name, probs in test_probs_tta.items()}
    else:
    ...
    if lightweight:
        ...
        if pseudo_enabled_this_run:
            ...
            if CFG.tta_gate_enabled:
                pseudo_test_avg = {name: gate_tta_probs(probs) for name, probs in pseudo_test_probs.items()}
            else:
    ...
    submission.to_csv("submission_final_team_v19.csv", index=False)
    ...
    pseudo_df.to_csv("v19_pseudo_labels_consensus.csv", index=False)
    ...
    with open("v19_oof_metrics.json", "w", encoding="utf-8") as f:
    ...
    manifest = {
        ...
        "outputs": {
            "final_submission": "submission_final_team_v19.csv",
            "metrics": "v19_oof_metrics.json",
            "pseudo_labels": "v19_pseudo_labels_consensus.csv",
        },
    }

    with open("v19_run_manifest.json", "w", encoding="utf-8") as f:
    ...
    print("Generated: submission_final_team_v19.csv")
    print("Generated: v19_oof_metrics.json")
    print("Generated: v19_run_manifest.json")
