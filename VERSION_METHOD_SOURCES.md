...
| `final_breakthrough_v25_moe_router.py` | Introduce MoE-style router over weighted/meta/consistency experts | Sparse MoE routing idea (e.g., Switch Transformers `2101.03961`) + FER multi-view consistency `2404.10904` | Indirect | Initial run stayed in CPU fallback branch; router path not effectively exercised |
| `final_breakthrough_v26_lightweight_moe.py` | Fix CPU branch to allow lightweight full-stack path; evaluate MoE gate in full pipeline | Calibration `1706.04599`, FER multi-view consistency `2404.10904`, MoE routing `2101.03961` | Indirect | Router was executed and rejected by OOF criterion (`router_oof < hybrid_oof`), so final prediction remained hybrid stack |
