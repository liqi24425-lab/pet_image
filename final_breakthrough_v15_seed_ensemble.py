...
def maybe_trackio_init(run_name: str):
    if trackio is None:
        return False
    try:
        trackio.init(project="pet-expression-v15", run_name=run_name, config=asdict(CFG))
        return True
    except Exception:
        return False
...
def main() -> None:
    ...
    run_id = f"v15-{now_utc.strftime('%Y%m%d-%H%M%S')}"
    run_hash = config_hash()

    device = torch.device("cuda" if torch.cuda.is_available() else ("mps" if torch.backends.mps.is_available() else "cpu"))
    print(f"Running v15 on {device}; run_id={run_id}; cfg={run_hash}")
    ...
    submission.to_csv("submission_final_team_v15.csv", index=False)
    ...
    pseudo_df.to_csv("v15_pseudo_labels_consensus.csv", index=False)
    ...
    with open("v15_oof_metrics.json", "w", encoding="utf-8") as f:
    ...
    manifest = {
        ...
        "outputs": {
            "final_submission": "submission_final_team_v15.csv",
            "metrics": "v15_oof_metrics.json",
            "pseudo_labels": "v15_pseudo_labels_consensus.csv",
        },
    }

    with open("v15_run_manifest.json", "w", encoding="utf-8") as f:
    ...
    print("Generated: submission_final_team_v15.csv")
    print("Generated: v15_oof_metrics.json")
    print("Generated: v15_run_manifest.json")
