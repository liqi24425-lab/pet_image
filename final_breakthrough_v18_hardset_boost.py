...
class V10Config:
    ...
    hard_boost_enabled: bool = True
    hard_boost_alpha: float = 0.6
    hard_boost_max: float = 2.0
...
def fit_oof_and_full(
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y), start=1):
        sw = None if sample_weight is None else sample_weight[tr_idx]
        model = train_linear_probe(X[tr_idx], y[tr_idx], num_classes, device, sample_weight=sw)
...
def build_hard_sample_weights(oof_by_model: Dict[str, np.ndarray]) -> np.ndarray:
    ...
    preds = {k: np.argmax(v, axis=1) for k, v in oof_by_model.items()}
    disagree = (
        (preds["dino"] != preds["effnet"]).astype(np.float32)
        + (preds["dino"] != preds["clip"]).astype(np.float32)
        + (preds["effnet"] != preds["clip"]).astype(np.float32)
    ) / 3.0

    hardness = 0.6 * ent + 0.4 * disagree
    w = 1.0 + CFG.hard_boost_alpha * hardness
    return np.clip(w.astype(np.float32), 1.0, CFG.hard_boost_max)
...
def main() -> None:
    ...
    run_id = f"v18-{now_utc.strftime('%Y%m%d-%H%M%S')}"
    ...
    print(f"Running v18 on feature={feature_device}, train={train_device}; run_id={run_id}; cfg={run_hash}")
    ...
    tta_keys = ["orig"] if lightweight else ["orig", "flip"]
    ...
    for name in ["dino", "effnet", "clip"]:
        print(f"Training stage1 OOF ensemble for {name}")
        oof, _ = fit_oof_and_full_ensemble(
            train_features[name],
            y,
            num_classes,
            train_device,
    ...
    hard_weights = np.ones(len(y), dtype=np.float32)
    if CFG.hard_boost_enabled:
        hard_weights = build_hard_sample_weights(oof_stage1)
        print(
            f"Hard-set weight stats: min={hard_weights.min():.3f}, "
            f"mean={hard_weights.mean():.3f}, max={hard_weights.max():.3f}"
        )

    oof_by_model = {}
    full_models = {}
    for name in ["dino", "effnet", "clip"]:
        print(f"Training stage2 boosted OOF ensemble for {name}")
        oof, full_model = fit_oof_and_full_ensemble(
            ...
            train_device,
            seeds=CFG.probe_seeds,
            sample_weight=hard_weights if CFG.hard_boost_enabled else None,
        )
    ...
    if lightweight:
        # CPU fallback: keep v18 hard-boost core and emit tuned weighted ensemble directly.
        stack_oof_acc = float(
            accuracy_score(
                y,
                np.argmax(
    ...
    submission.to_csv("submission_final_team_v18.csv", index=False)
    ...
    pseudo_df.to_csv("v18_pseudo_labels_consensus.csv", index=False)
    ...
    metrics = {
        ...
        "probe_seeds": list(CFG.probe_seeds),
        "hard_boost_enabled": CFG.hard_boost_enabled,
        "hard_weight_mean": float(hard_weights.mean()),
    ...
    with open("v18_oof_metrics.json", "w", encoding="utf-8") as f:
    ...
    manifest = {
        ...
        "outputs": {
            "final_submission": "submission_final_team_v18.csv",
            "metrics": "v18_oof_metrics.json",
            "pseudo_labels": "v18_pseudo_labels_consensus.csv",
        },
    }

    with open("v18_run_manifest.json", "w", encoding="utf-8") as f:
    ...
    print("Generated: submission_final_team_v18.csv")
    print("Generated: v18_oof_metrics.json")
    print("Generated: v18_run_manifest.json")
