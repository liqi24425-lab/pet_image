...
- `v15`：线上已验证与 `v10` 同分 `0.92666`，属于平台期（稳定但无突破）。
- `v16`：引入自适应 ROI（解决“头不在中心”问题），但离线指标与 `v15` 持平，暂未突破。
...
- `v15` 已在 2026-03-06 验证公开榜 `0.92666`，与 `v10` 持平，不构成新 best。
...
1. **更强脸部区域策略（ROI）**
在特征提取前先做更紧的人脸区域裁剪/对齐，减少背景捷径特征。
2. **物种感知分支（Species-aware）**
先做宠物物种粗分类，再进表情头，降低跨物种干扰。
3. **困难样本去噪闭环**
把 `v10/v13/v15` 分歧样本做固定 hard set，进行定向重加权或半自动复核。
