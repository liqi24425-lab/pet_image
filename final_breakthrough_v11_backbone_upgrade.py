...
def load_backbones(device: torch.device) -> Dict[str, BackboneSpec]:
    ...
    # Upgraded CNN backbone: EfficientNetV2-L
    eff = efficientnet_v2_l(weights=EfficientNet_V2_L_Weights.DEFAULT)
    ...
    # Upgraded CLIP vision backbone: ViT-L/14
...
def maybe_trackio_init(run_name: str):
    if trackio is None:
        return False
    try:
        trackio.init(project="pet-expression-v11", run_name=run_name, config=asdict(CFG))
        return True
    except Exception:
        return False
...
def main() -> None:
    ...
    run_id = f"v11-{now_utc.strftime('%Y%m%d-%H%M%S')}"
    run_hash = config_hash()

    device = torch.device("cuda" if torch.cuda.is_available() else ("mps" if torch.backends.mps.is_available() else "cpu"))
    print(f"Running v11 on {device}; run_id={run_id}; cfg={run_hash}")
    ...
    submission.to_csv("submission_final_team_v11.csv", index=False)
    ...
    pseudo_df.to_csv("v11_pseudo_labels_consensus.csv", index=False)
    ...
    with open("v11_oof_metrics.json", "w", encoding="utf-8") as f:
    ...
    manifest = {
        ...
        "outputs": {
            "final_submission": "submission_final_team_v11.csv",
            "metrics": "v11_oof_metrics.json",
            "pseudo_labels": "v11_pseudo_labels_consensus.csv",
        },
    }

    with open("v11_run_manifest.json", "w", encoding="utf-8") as f:
    ...
    print("Generated: submission_final_team_v11.csv")
    print("Generated: v11_oof_metrics.json")
    print("Generated: v11_run_manifest.json")
