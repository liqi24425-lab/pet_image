...
def load_backbones(device: torch.device) -> Dict[str, BackboneSpec]:
    ...
    clip = CLIPVisionModelWithProjection.from_pretrained("openai/clip-vit-large-patch14").to(device).eval()
...
def blend_by_weights(prob_by_model: Dict[str, np.ndarray], weights: Dict[str, float]) -> np.ndarray:
    return (
        weights["dino"] * prob_by_model["dino"]
        + weights["effnet"] * prob_by_model["effnet"]
        + weights["clip"] * prob_by_model["clip"]
    )
...
def build_meta_true_oof(X_meta: np.ndarray, y: np.ndarray):
    meta_model, meta_best_params = build_meta_learner(X_meta, y)

    skf = StratifiedKFold(n_splits=CFG.n_splits, shuffle=True, random_state=CFG.seed)
    meta_oof_prob = np.zeros((len(y), len(np.unique(y))), dtype=np.float32)

    for fold, (tr_idx, va_idx) in enumerate(skf.split(X_meta, y), start=1):
        fold_model = clone(meta_model)
        fold_model.fit(X_meta[tr_idx], y[tr_idx])
        meta_oof_prob[va_idx] = fold_model.predict_proba(X_meta[va_idx])
        print(f"Meta fold {fold}/{CFG.n_splits} done")

    meta_oof_pred = np.argmax(meta_oof_prob, axis=1)
    meta_oof_acc = float(accuracy_score(y, meta_oof_pred))
    meta_oof_f1 = float(f1_score(y, meta_oof_pred, average="macro"))
    return meta_model, meta_best_params, meta_oof_prob, meta_oof_acc, meta_oof_f1


def tune_hybrid_alpha(meta_oof_prob: np.ndarray, weighted_oof_prob: np.ndarray, y: np.ndarray):
    best_alpha = 0.5
    best_acc = -1.0
    best_f1 = -1.0
    for alpha in np.arange(0.0, 1.01, 0.05):
        prob = alpha * meta_oof_prob + (1.0 - alpha) * weighted_oof_prob
        ...
        if (acc > best_acc) or (abs(acc - best_acc) < 1e-9 and f1 > best_f1):
            ...
            best_acc = acc
            best_f1 = f1
    print(f"Tuned hybrid alpha={best_alpha:.2f}, OOF acc={best_acc:.4f}, macro_f1={best_f1:.4f}")
    return best_alpha, best_acc, best_f1
...
def maybe_trackio_init(run_name: str):
    if trackio is None:
        return False
    try:
        trackio.init(project="pet-expression-v13", run_name=run_name, config=asdict(CFG))
        return True
    except Exception:
        return False
...
def main() -> None:
    ...
    run_id = f"v13-{now_utc.strftime('%Y%m%d-%H%M%S')}"
    run_hash = config_hash()

    device = torch.device("cuda" if torch.cuda.is_available() else ("mps" if torch.backends.mps.is_available() else "cpu"))
    print(f"Running v13 on {device}; run_id={run_id}; cfg={run_hash}")
    ...
    weighted_oof_prob = blend_by_weights(oof_by_model, tuned_weights)
    weighted_oof_pred = np.argmax(weighted_oof_prob, axis=1)
    weighted_oof_acc = float(accuracy_score(y, weighted_oof_pred))
    weighted_oof_f1 = float(f1_score(y, weighted_oof_pred, average="macro"))

    oof_stack_input = np.hstack([oof_by_model["dino"], oof_by_model["effnet"], oof_by_model["clip"]])
    meta_model, meta_best_params, meta_oof_prob, meta_oof_acc, meta_oof_f1 = build_meta_true_oof(oof_stack_input, y)

    hybrid_alpha, stack_oof_acc, stack_oof_f1 = tune_hybrid_alpha(meta_oof_prob, weighted_oof_prob, y)
    ...
    maybe_trackio_log(
        trk,
        {
            ...
            "weighted_oof_macro_f1": weighted_oof_f1,
            "meta_true_oof_acc": meta_oof_acc,
            "meta_true_oof_macro_f1": meta_oof_f1,
    ...
    weighted_prob = blend_by_weights(test_probs_by_model, tuned_weights)
    ...
    meta_prob = meta_model.predict_proba(stack_test_input)
    stack_prob = hybrid_alpha * meta_prob + (1.0 - hybrid_alpha) * weighted_prob
    ...
    if pseudo_enabled_this_run:
        ...
        pseudo_weighted_prob = blend_by_weights(pseudo_test_avg, tuned_weights)
        pseudo_stack_input = np.hstack([pseudo_test_avg["dino"], pseudo_test_avg["effnet"], pseudo_test_avg["clip"]])
        pseudo_meta_prob = meta_model.predict_proba(pseudo_stack_input)
        pseudo_final_prob = hybrid_alpha * pseudo_meta_prob + (1.0 - hybrid_alpha) * pseudo_weighted_prob
        final_pred = np.argmax(pseudo_final_prob, axis=1)
    ...
    submission.to_csv("submission_final_team_v13.csv", index=False)
    ...
    pseudo_df.to_csv("v13_pseudo_labels_consensus.csv", index=False)
    ...
    with open("v13_oof_metrics.json", "w", encoding="utf-8") as f:
    ...
    manifest = {
        ...
        "outputs": {
            "final_submission": "submission_final_team_v13.csv",
            "metrics": "v13_oof_metrics.json",
            "pseudo_labels": "v13_pseudo_labels_consensus.csv",
        },
    }

    with open("v13_run_manifest.json", "w", encoding="utf-8") as f:
    ...
    print("Generated: submission_final_team_v13.csv")
    print("Generated: v13_oof_metrics.json")
    print("Generated: v13_run_manifest.json")
